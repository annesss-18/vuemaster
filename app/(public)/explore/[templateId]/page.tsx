// app/(public)/explore/[templateId]/page.tsx
import { getTemplateById } from "@/lib/actions/general.action";
import DisplayTechIcons from "@/components/DisplayTechIcons";
import CompanyLogo from "@/components/CompanyLogo";
import AuthAwareStartButton from "@/components/AuthAwareStartButton";
import { Briefcase, Target, ArrowLeft } from "lucide-react";
import Link from "next/link";

const PublicTemplatePage = async ({ params }: { params: Promise<{ templateId: string }> }) => {
    ...
    if (!template) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
                ...
                <Link href="/explore" className="btn-primary">Go Back</Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto py-10 px-6 space-y-8 animate-fadeIn">
            <Link href="/explore" className="text-primary-300 hover:text-primary-200 flex items-center gap-2 mb-6 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="card-border animate-slideInUp" style={{ animationDelay: "0.3s" }}>
                        <div className="card !p-6 space-y-4">
                            <h2 className="text-xl font-semibold text-light-100">Ready to Practice?</h2>
                            <AuthAwareStartButton templateId={template.id} />
                        </div>
                    </div>
                </div>
            </div>
